    ...
    <div className="pt-16 flex-1 p-4 md:max-w-6xl md:mx-auto h-[calc(100vh-62px)]">
      ...
      <div className="mt-8 h-[500px]">
        ...
        <div className="w-full mt-4 grid grid-cols-4 gap-4">
          {chats?.map(card => (
            <div className="h-[300px] bg-gray-50 relative group/card  dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] rounded-xl p-6 border" key={card.id}><span>{card.fileName}</span></div>
          ))}
        </div>
      </div>
    </div>
