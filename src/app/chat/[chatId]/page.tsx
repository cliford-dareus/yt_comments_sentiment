...
const Chat = async ({ params }: { params: { chatId: string } }) => {
  ...
  const [sentimentRows, initialMessages, { comments, stats }, clusters, triage] =
    await Promise.all([
      db
        .select()
        .from($sentiment)
      ...
      getThemeClusters(chat[0].id),
      getTriageInbox(chat[0].id),
    ]);
